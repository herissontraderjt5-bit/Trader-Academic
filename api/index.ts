...
async function startServer() {
  try {
    ...
    app.post("/api/signals/generate", authenticate, async (req: any, res: any) => {
      console.log(`[GENERATOR] Início do pedido de sinal por: ${req.user.login}`);
      try {
        ...
        console.log(`[GENERATOR] Usuário: ${user?.login}, Role: ${req.user.role}, Plano: ${user?.plan_name}`);

        const hasPlan = user && user.plan_name && user.plan_name !== 'Plano Free' && user.access_days > 0;

        if (req.user.role !== 'admin' && !hasPlan) {
          console.warn(`[GENERATOR] Acesso Negado para ${req.user.login}`);
        ...
        console.log(`[GENERATOR] Sinal de ${signal.asset} gerado com ID: ${fullSignal.id}`);
        broadcastSignal(fullSignal);
        res.json(fullSignal);
      } catch (e: any) {
        console.error('[GENERATOR] ERRO CRÍTICO:', e);
        res.status(500).json({ error: e.message || "Erro interno ao gerar sinal" });
      }
    });
